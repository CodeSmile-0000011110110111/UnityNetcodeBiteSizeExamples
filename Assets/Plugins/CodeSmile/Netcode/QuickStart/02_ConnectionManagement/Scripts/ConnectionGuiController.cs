	...
	public sealed partial class ConnectionGuiController : MonoBehaviour
	{
		...
		private void ShutdownNetwork()
		{
			...
			if (netMan != null)
			{
				...
				FindObjectOfType<NetworkDisconnectManager>().NetworkShutdown();
				UpdateGuiActiveState();
			}
		}
		...
		private void KickClients(int kickThisManyClients)
		{
			...
			var connMan = FindObjectOfType<NetworkDisconnectManager>();
			...
			for (var i = clientCount - 1; i >= 0; i--)
			{
				...
				connMan.KickClient(clientId, NetworkDisconnectManager.KickReason.ServerAuthority);
